...
type SendChannel<'a> = (
    crossbeam::channel::Sender<ProtoString<'a>>,
    crossbeam::channel::Receiver<ProtoString<'a>>,
);
...
pub fn read_perms(input: &str, outfile: &str) -> StrResult {
    ...
    let (etx, erx): (
        crossbeam::channel::Sender<StrResult>,
        crossbeam::channel::Receiver<StrResult>,
    ) = unbounded();
...
fn parse_input(
    input: &str,
    tx: crossbeam::channel::Sender<ProtoString>,
    etx: crossbeam::channel::Sender<StrResult>,
) {
    let mut instr: String = String::new();

    if input == "-" {
        if let Err(x) = std::io::stdin().read_to_string(&mut instr) {
            etx.send(Err(logging::format_log(file!(), line!(), &x.to_string())))
                .unwrap();
            return;
        }
    } else {
    ...
    for i in instr.split('\n') {
        match tx.send(i.to_string().into()) {
            Ok(_) => (),
            Err(x) => {
                etx.send(Err(logging::format_log(file!(), line!(), &x.to_string())))
                    .unwrap();
...
fn lookup_values<'a>(
    output: Arc<Mutex<&mut ProtoHashMap<'a>>>,
    rx: crossbeam::channel::Receiver<Cow<'a, str>>,
) {
    ...
    loop {
        ...
        cache_vec.push((file_path, perm));

        if let Ok(mut x) = output.try_lock() {
            while let Some(p) = cache_vec.pop() {
                x.insert(p.0, p.1);
            }
        }
    }
...
fn write_values_stdout(input: ProtoHashMap) -> StrResult {
    let o = perms::permissions { permission: input };
...
fn write_values(input: ProtoHashMap, output: &str) -> StrResult {
    ...
    if p.is_file() {
        ...
        return Err(logging::format_log(
            file!(),
            line!(),
            &format!("{} is a directory!", output),
        ));
    }
    let mut outfile = match Encoder::new(
        BufWriter::new(
            match OpenOptions::new().create(true).truncate(true).write(true).open(output) {
                Ok(x) => x,
                Err(x) => {
                    return Err(logging::format_log(file!(), line!(), &x.to_string()));
                }
